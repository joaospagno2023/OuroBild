...
from app.services.setup.setup_project_preparer import SetupProjectPreparer
from app.services.setup.setup_workspace_service import SetupWorkspaceService
...
def create_project() -> Project:
    ...
    return Project(
        ...
        solution_path=r"C:\Projetos\Projeto\Projeto.sln",
        project_path=r"C:\Projetos\Projeto\Projeto.csproj",
        compilation_target=CompilationTarget.PROJECT,
        compilation_engine=CompilationEngine.MSBUILD,
        publish_path=r"C:\Projetos\Projeto\bin\Release",
        publish_profile=None,
        aip_path=r"C:\Projetos\Projeto\Setup\Projeto.aip",
        visualstudio_setup_path=None,
        output_msi=r"C:\Setups\Projeto.Setup.msi",
...
def create_workspace_context() -> WorkspaceContext:
    """
    Cria um WorkspaceContext real e mínimo.

    O DefaultSetupOrchestrator acessa diretamente:

        - workspace.project;
        - workspace.project_file;
        - workspace.environment.root_path.

    Por isso o teste utiliza os modelos reais.
    """
...
def test_deve_executar_setup_advanced_installer():
    ...
    orchestrator = create_orchestrator(
        ...
        definition_loader=visual_studio_definition_loader,
        advanced_installer_definition_loader=(
            advanced_installer_definition_loader
        ),
        setup_factory=setup_factory,
        setup_project_preparer=setup_project_preparer,
        setup_workspace_service=setup_workspace_service,
    )
    ...
    assert result.project_id == "teste"

    assert result.output_msi == paths.output_msi
...
def test_deve_retornar_falha_quando_loader_advanced_installer_falhar():
    ...
    advanced_installer_definition_loader.load.side_effect = (
        ValueError("Definition inválida.")
    )
