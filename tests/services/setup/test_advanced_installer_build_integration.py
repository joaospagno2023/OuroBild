"""
--------------------------------------------------------------------
Projeto : OuroBuild
Arquivo : test_advanced_installer_build_integration.py
Descrição : Teste de integração do Build do Advanced Installer
            utilizando o AIP real do LinkPagamento.
--------------------------------------------------------------------
"""

import shutil

from pathlib import Path

from app.models.process.command import (
    Command,
)

from app.models.process.command_argument import (
    CommandArgument,
)

from app.models.process.process_status import (
    ProcessStatus,
)

from app.services.process_service import (
    DefaultProcessService,
)

from app.services.cleanup.build_artifact_cleanup_factory import (
    BuildArtifactCleanupFactory,
)


AIP_ROOT = Path(
    r"C:\DvpLocal\WorkSpaceTFS"
    r"\Transferencia de Arquivo"
    r"\TransferenciaDeArquivos"
    r"\Setups"
    r"\Installers"
    r"\Projects"
)


AIP_PATH = (
    AIP_ROOT
    / "OuroNet.WinServiceLinkPagamento.aip"
)


PUBLISH_PATH = Path(
    r"C:\DvpLocal\WorkSpaceTFS"
    r"\OuroNet"
    r"\Scc"
    r"\Producao"
    r"\OuroNet"
    r"\02-Source"
    r"\01-Client"
    r"\OuroNet.Client.WinService.LinkPagamento"
    r"\bin"
    r"\Release"
)


ADVANCED_INSTALLER_PATH = Path(
    r"C:\Program Files (x86)"
    r"\Caphyon"
    r"\Advanced Installer 21.9"
    r"\bin"
    r"\x86"
    r"\AdvancedInstaller.com"
)


TEST_VERSION = "99.99.99.0"


def test_deve_executar_refresh_sync_e_build_no_aip_real(
    tmp_path: Path,
):
    """
    Deve executar RefreshSync e Build utilizando
    o AIP real do LinkPagamento.

    O AIP original nunca deve ser alterado.
    """

    #
    # ============================================================
    # 1. Validar AIP.
    # ============================================================
    #

    if not AIP_PATH.exists():

        raise FileNotFoundError(
            "AIP do LinkPagamento não encontrado:\n"
            f"{AIP_PATH}"
        )

    if not AIP_PATH.is_file():

        raise ValueError(
            "O AIP do LinkPagamento não é um arquivo:\n"
            f"{AIP_PATH}"
        )

    #
    # ============================================================
    # 2. Validar Release.
    # ============================================================
    #

    if not PUBLISH_PATH.exists():

        raise FileNotFoundError(
            "Pasta Release do LinkPagamento não encontrada:\n"
            f"{PUBLISH_PATH}"
        )

    if not PUBLISH_PATH.is_dir():

        raise ValueError(
            "A pasta Release do LinkPagamento "
            "não é um diretório:\n"
            f"{PUBLISH_PATH}"
        )

    #
    # ============================================================
    # 3. Validar Advanced Installer.
    # ============================================================
    #

    if not ADVANCED_INSTALLER_PATH.exists():

        raise FileNotFoundError(
            "AdvancedInstaller.com não encontrado:\n"
            f"{ADVANCED_INSTALLER_PATH}"
        )

    if not ADVANCED_INSTALLER_PATH.is_file():

        raise ValueError(
            "AdvancedInstaller.com não é um arquivo:\n"
            f"{ADVANCED_INSTALLER_PATH}"
        )

    #
    # ============================================================
    # 4. Criar cópia temporária do AIP.
    # ============================================================
    #

    temporary_aip = (
        tmp_path
        / AIP_PATH.name
    )

    shutil.copy2(
        AIP_PATH,
        temporary_aip,
    )

    #
    # ============================================================
    # 5. Criar uma cópia temporária dos Prerequisites.
    #
    # O AIP possui referências relativas aos pré-requisitos.
    # Portanto, a estrutura de Prerequisites precisa existir
    # junto ao AIP temporário.
    # ============================================================
    #

    prerequisites_source_path = (
        AIP_ROOT
        / "Prerequisites"
    )

    if not prerequisites_source_path.exists():

        raise FileNotFoundError(
            "Pasta Prerequisites do AIP não encontrada:\n"
            f"{prerequisites_source_path}"
        )

    if not prerequisites_source_path.is_dir():

        raise ValueError(
            "A pasta Prerequisites do AIP não é um diretório:\n"
            f"{prerequisites_source_path}"
        )

    temporary_prerequisites_path = (
        tmp_path
        / "Prerequisites"
    )

    shutil.copytree(
        prerequisites_source_path,
        temporary_prerequisites_path,
    )

    required_prerequisites = [
        (
            temporary_prerequisites_path
            / ".NET Framework 4.0"
            / "dotNetFx40_Full_x86_x64.exe"
        ),
        (
            temporary_prerequisites_path
            / ".NET Framework 4.5"
            / "dotnetfx45_full_x86_x64.exe"
        ),
    ]

    missing_prerequisites = [
        path
        for path in required_prerequisites
        if not path.is_file()
    ]

    assert not missing_prerequisites, (
        "Recursos obrigatórios do AIP não foram encontrados "
        "no diretório temporário de Prerequisites.\n\n"
        "Arquivos ausentes:\n"
        + "\n".join(
            str(path)
            for path in missing_prerequisites
        )
    )

    #
    # ============================================================
    # 18. Criar uma cópia temporária do Release.
    #
    # O Cleanup é executado sobre o workspace de trabalho, nunca
    # diretamente sobre o Release real do ambiente.
    # ============================================================
    #

    temporary_publish_path = (
        tmp_path
        / "Release"
    )

    shutil.copytree(
        PUBLISH_PATH,
        temporary_publish_path,
    )

    #
    # ============================================================
    # 18. Executar Cleanup do projeto.
    #
    # Este teste precisa reproduzir o fluxo real: o artefato
    # publicado passa pelo Cleanup antes de ser utilizado pelo
    # sincronizador do AIP.
    #
    # Para o projeto LinkPagamento, a regra global REMOVE * deve
    # eliminar arquivos como libSkiaSharp.so antes do RefreshSync.
    # ============================================================
    #

    cleanup_factory = BuildArtifactCleanupFactory()
    cleanup_service = cleanup_factory.create(
        project_id="linkpagamento",
    )

    cleanup_service.execute(
        workspace_path=temporary_publish_path,
        project_id="linkpagamento",
    )

    remaining_skia_files = list(
        temporary_publish_path.rglob(
            "libSkiaSharp.so"
        )
    )

    assert not remaining_skia_files, (
        "O Cleanup do LinkPagamento não removeu os arquivos "
        "libSkiaSharp.so.\n\n"
        "Arquivos encontrados após o Cleanup:\n"
        + "\n".join(
            str(path)
            for path in remaining_skia_files
        )
    )

    #
    # ============================================================
    # 18. Tornar a cópia gravável.
    #
    # Arquivos provenientes do TFS/TFVC podem estar
    # somente leitura.
    # ============================================================
    #

    temporary_aip.chmod(
        temporary_aip.stat().st_mode
        | 0o200
    )

    #
    # ============================================================
    # 18. Preparar o AIP.
    #
    # Utilizamos o mesmo sincronizador conectado ao
    # AdvancedInstallerService em produção (Parser +
    # Comparator + Modifier), garantindo que ROWs de
    # arquivos obsoletos (REMOVE) sejam removidas antes
    # do RefreshSync/Build nativos do Advanced Installer.
    # ============================================================
    #

    from app.services.setup.advanced_installer_aip_file_parser import (
        AdvancedInstallerAipFileParser,
    )

    from app.services.setup.advanced_installer_aip_file_comparator import (
        AdvancedInstallerAipFileComparator,
    )

    from app.services.setup.advanced_installer_aip_modifier import (
        AdvancedInstallerAipModifier,
    )

    from app.services.setup.advanced_installer_aip_synchronizer import (
        AdvancedInstallerAipSynchronizer,
    )

    aip_synchronizer = (
        AdvancedInstallerAipSynchronizer(
            parser=(
                AdvancedInstallerAipFileParser()
            ),
            comparator=(
                AdvancedInstallerAipFileComparator()
            ),
            modifier=(
                AdvancedInstallerAipModifier()
            ),
        )
    )

    aip_synchronizer.synchronize(
        aip_path=temporary_aip,
        version=TEST_VERSION,
        publish_path=temporary_publish_path,
    )

    #
    # ============================================================
    # 18. Validar alteração da versão.
    # ============================================================
    #

    prepared_content = (
        temporary_aip.read_text(
            encoding="utf-8",
        )
    )

    assert (
        f'Value="{TEST_VERSION}"'
        in prepared_content
    )

    #
    # ============================================================
    # 18. Criar ProcessService real.
    # ============================================================
    #

    process_service = (
        DefaultProcessService()
    )

    #
    # ============================================================
    # 18. Executar RefreshSync.
    # ============================================================
    #

    refresh_sync_command = Command(
        executable=(
            ADVANCED_INSTALLER_PATH
        ),
        working_directory=(
            temporary_aip.parent
        ),
        arguments=[
            CommandArgument(
                value="/edit",
            ),
            CommandArgument(
                value=str(
                    temporary_aip
                ),
            ),
            CommandArgument(
                value="/RefreshSync",
            ),
        ],
    )

    print()
    print(
        "=" * 80
    )

    print(
        "[OuroBuild] ADVANCED INSTALLER - REFRESH SYNC"
    )

    print(
        "=" * 80
    )

    print(
        "[OuroBuild] Executável:"
    )

    print(
        f"[OuroBuild] {ADVANCED_INSTALLER_PATH}"
    )

    print(
        "[OuroBuild] AIP:"
    )

    print(
        f"[OuroBuild] {temporary_aip}"
    )

    print(
        "[OuroBuild] Origem:"
    )

    print(
        f"[OuroBuild] {temporary_publish_path}"
    )

    print(
        "[OuroBuild] Versão:"
    )

    print(
        f"[OuroBuild] {TEST_VERSION}"
    )

    print(
        "=" * 80
    )

    refresh_sync_result = (
        process_service.execute(
            refresh_sync_command,
        )
    )

    print()
    print(
        "[OuroBuild] RefreshSync ExitCode:"
    )

    print(
        f"[OuroBuild] "
        f"{refresh_sync_result.exit_code}"
    )

    print()
    print(
        "[OuroBuild] RefreshSync STDOUT:"
    )

    print(
        refresh_sync_result.stdout
    )

    print()
    print(
        "[OuroBuild] RefreshSync STDERR:"
    )

    print(
        refresh_sync_result.stderr
    )

    assert (
        refresh_sync_result.status
        == ProcessStatus.SUCCESS
    ), (
        "O Advanced Installer retornou erro "
        "durante o RefreshSync.\n\n"
        f"ExitCode: "
        f"{refresh_sync_result.exit_code}\n\n"
        f"STDOUT:\n"
        f"{refresh_sync_result.stdout}\n\n"
        f"STDERR:\n"
        f"{refresh_sync_result.stderr}"
    )

    #
    # ============================================================
    # 18. Executar Build.
    # ============================================================
    #

    build_command = Command(
        executable=(
            ADVANCED_INSTALLER_PATH
        ),
        working_directory=(
            temporary_aip.parent
        ),
        arguments=[
            CommandArgument(
                value="/build",
            ),
            CommandArgument(
                value=str(
                    temporary_aip
                ),
            ),
        ],
    )

    print()
    print(
        "=" * 80
    )

    print(
        "[OuroBuild] ADVANCED INSTALLER - BUILD"
    )

    print(
        "=" * 80
    )

    print(
        "[OuroBuild] Executável:"
    )

    print(
        f"[OuroBuild] {ADVANCED_INSTALLER_PATH}"
    )

    print(
        "[OuroBuild] AIP:"
    )

    print(
        f"[OuroBuild] {temporary_aip}"
    )

    print(
        "[OuroBuild] Comando:"
    )

    print(
        "[OuroBuild] "
        f'"{ADVANCED_INSTALLER_PATH}" '
        f'/build '
        f'"{temporary_aip}"'
    )

    print(
        "=" * 80
    )

    build_result = (
        process_service.execute(
            build_command,
        )
    )

    print()
    print(
        "[OuroBuild] Build ExitCode:"
    )

    print(
        f"[OuroBuild] "
        f"{build_result.exit_code}"
    )

    print()
    print(
        "[OuroBuild] Build STDOUT:"
    )

    print(
        build_result.stdout
    )

    print()
    print(
        "[OuroBuild] Build STDERR:"
    )

    print(
        build_result.stderr
    )

    #
    # ============================================================
    # 18. Validar Build.
    # ============================================================
    #

    assert (
        build_result.status
        == ProcessStatus.SUCCESS
    ), (
        "O Advanced Installer retornou erro "
        "durante o Build.\n\n"
        f"ExitCode: "
        f"{build_result.exit_code}\n\n"
        f"STDOUT:\n"
        f"{build_result.stdout}\n\n"
        f"STDERR:\n"
        f"{build_result.stderr}"
    )

    #
    # ============================================================
    # 18. Resolver MSI gerado pelo Advanced Installer.
    #
    # O nome/caminho do pacote é definido pelo próprio AIP.
    # Portanto, não devemos assumir um nome fixo para o MSI.
    # O Build informa o pacote gerado através de:
    #
    #     Building package: <caminho>
    #
    # ============================================================

    import re

    package_matches = re.findall(
        r"(?im)^\s*Building package:\s*(.+?\.msi)\s*$",
        build_result.stdout or "",
    )

    output_msi = None

    if package_matches:

        package_path_text = package_matches[-1].strip().strip('"')
        package_path = Path(package_path_text)

        if not package_path.is_absolute():
            package_path = (
                temporary_aip.parent
                / package_path
            )

        output_msi = package_path

    #
    # Caso o Advanced Installer não tenha informado o pacote no
    # STDOUT, fazemos uma busca de segurança dentro do diretório
    # temporário.
    #
    if output_msi is None:

        generated_files = list(
            tmp_path.rglob(
                "*.msi"
            )
        )

        if generated_files:
            output_msi = generated_files[0]

    #
    # ============================================================
    # 18. Validar MSI.
    # ============================================================

    assert output_msi is not None, (
        "O Build terminou com sucesso, "
        "porém nenhum arquivo MSI foi encontrado.\n\n"
        f"Temporary AIP: {temporary_aip}\n\n"
        "MSIs encontrados no diretório temporário:\n"
        + "\n".join(
            str(path)
            for path in tmp_path.rglob("*.msi")
        )
        + "\n\n"
        f"STDOUT:\n{build_result.stdout}\n\n"
        f"STDERR:\n{build_result.stderr}"
    )

    assert output_msi.exists(), (
        "O Advanced Installer informou um MSI no Build, "
        "mas o arquivo não existe no caminho informado.\n\n"
        f"MSI informado: {output_msi}\n\n"
        f"Temporary AIP: {temporary_aip}\n\n"
        "MSIs encontrados no diretório temporário:\n"
        + "\n".join(
            str(path)
            for path in tmp_path.rglob("*.msi")
        )
    )

    assert output_msi.is_file()

    assert (
        output_msi.stat().st_size > 0
    )

    #
    # ============================================================
    # 18. O AIP temporário deve continuar existindo.
    # ============================================================
    #

    assert (
        temporary_aip.exists()
    )

    #
    # ============================================================
    # 18. O AIP original deve continuar intacto.
    # ============================================================
    #

    original_content = (
        AIP_PATH.read_text(
            encoding="utf-8",
        )
    )

    assert (
        f'Value="{TEST_VERSION}"'
        not in original_content
    )

    #
    # ============================================================
    # 19. Diagnóstico final.
    # ============================================================
    #

    print()
    print(
        "=" * 80
    )

    print(
        "[OuroBuild] BUILD CONCLUÍDO"
    )

    print(
        "=" * 80
    )

    print(
        "[OuroBuild] RefreshSync: OK"
    )

    print(
        "[OuroBuild] Build: OK"
    )

    print(
        "[OuroBuild] MSI:"
    )

    print(
        f"[OuroBuild] {output_msi}"
    )

    print(
        "[OuroBuild] "
        "O AIP original permaneceu intacto."
    )

    print(
        "=" * 80
    )